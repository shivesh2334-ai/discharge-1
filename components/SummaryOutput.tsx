      ...
      >
        <div>
          ...
          <div style={{ color: '#94a3b8', fontSize: 12, marginTop: 2 }}>
            AI-generated · Requires physician sign-off
          </div>
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={onEdit} style={btnStyle('#334155')}>
            ✏️ Edit
          </button>
          <button onClick={() => window.print()} style={btnStyle('#059669')}>
            🖨️ Print
          </button>
          <button onClick={onReset} style={btnStyle('#e85d5d')}>
            ↩ Reset
          </button>
        </div>
      </div>
        ...
        <SummarySection title="Procedures Performed" color={SECTION_COLORS.procedures}>
          <ul style={{ margin: 0, padding: '0 0 0 18px' }}>
            {summary.procedures?.map((p, i) => (
              <li key={i} style={{ color: '#475569', fontSize: 13, marginBottom: 4 }}>
                {p}
              </li>
            ))}
          </ul>
          ...
          <SummarySection title="Follow-up Plan" color={SECTION_COLORS.followup}>
            <ul style={{ margin: 0, padding: '0 0 0 18px' }}>
              {summary.followUp?.map((f, i) => (
                <li key={i} style={{ color: '#475569', fontSize: 13, marginBottom: 5 }}>
                  {f}
                </li>
              ))}
            </ul>
